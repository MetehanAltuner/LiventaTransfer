...
public sealed class DriverController : ControllerBase
{
    ...
    public async Task<IActionResult> Contact(Guid publicId, CancellationToken ct)
    {
        var r = await _svc.MarkContactedAsync(publicId, User.GetUserId(), ct);
        return StatusCode(r.StatusCode, r);
    }

    /// <summary>Yola Çıktım — sürücünün yola çıktığını işaretler.</summary>
    [HttpPost("jobs/{publicId:guid}/depart")]
    public async Task<IActionResult> Depart(Guid publicId, CancellationToken ct)
    {
        var r = await _svc.MarkDepartedAsync(publicId, User.GetUserId(), ct);
        return StatusCode(r.StatusCode, r);
    }

    /// <summary>Aldım — ilgili durakta yolcuyu alındı olarak işaretler.</summary>
    [HttpPost("jobs/{publicId:guid}/stops/{stopId:long}/pickup")]
    public async Task<IActionResult> Pickup(Guid publicId, long stopId, CancellationToken ct)
    {
        var r = await _svc.MarkStopPickedUpAsync(publicId, stopId, User.GetUserId(), ct);
        return StatusCode(r.StatusCode, r);
    }
    ...
    public async Task<IActionResult> Dropoff(Guid publicId, long stopId, CancellationToken ct)
    {
        var r = await _svc.MarkStopDroppedOffAsync(publicId, stopId, User.GetUserId(), ct);
        return StatusCode(r.StatusCode, r);
    }
}
