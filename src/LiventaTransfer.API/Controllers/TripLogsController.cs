...
public sealed class TripLogsController : ControllerBase
{
    ...
    [HttpPost("jobs/{jobId:guid}/trip-log")]
    public async Task<IActionResult> Create(Guid jobId, [FromBody] CreateTripLogRequest request, CancellationToken ct)
    {
        var r = await _svc.CreateAsync(jobId, request, ct);
        return StatusCode(r.StatusCode, r);
    }

    [HttpPut("trip-logs/{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateTripLogRequest request, CancellationToken ct)
    {
